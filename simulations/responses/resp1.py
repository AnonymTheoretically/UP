...
resp1 -> stuck on the main chain

we will be recording the avenger's profits
'''
...
class StateGraph:
    ...
    def findAllPathsUtil(self, s, visited, path):
        ...
        if s in self.finishline:
            # print(path)
            # ignore when the fork wins
            if s>= self.depth*(self.depth-1):
                        ...
                        if len(path) == 1: # just starting
                            ...
                            if vt - path[-1] == 1: # the fork extends by one
                                ...
                                if self.signal[s] == 1: # honest miners already on the fork
                                    self.shift[vt] = p * max((1-self.PoF[s]-self.alpha),0)
                                    self.signal[vt] = 1
                                else:
                                    if m < n:
                                        self.shift[vt] = p*max((1-self.PoF[s]-self.beta-self.alpha),0) + self.beta
                                        self.signal[vt] = 1 # update the signal
                                    else:
                                        self.shift[vt] = p*max((1-self.PoF[s]-self.beta-self.alpha),0)
                                        self.signal[vt] = 0
                                # calculate the mining poewr on fork

...
resp3 -> honest mining
part 2 -> main chain wins

final returns = OUTPUT combined with OUTPUT from resp3 * 0.45/0.55
'''
import sys
...
class StateGraph:
                    ...
                    if vt not in self.finishline:
                        ...
                        if len(path) == 1: # just starting
                            ...
                            if vt - path[-1] == 1: # the fork extends by one                                
                                ...
                                # calculate the mining poewr on fork
                                self.PoF[vt] = self.PoF[s] + self.shift[vt]
                                if self.PoF[vt] < self.alpha and self.PoF[vt] > self.alpha-0.02:
                                    self.PoF[vt] = self.alpha
                                self.roundRet[vt] = 1
                        else:
    ...
    def calculateProfits(self, m, n):
        ...
        self.roundRet[self.src] = self.beta
